...
const config = {
	ENTITY_NAME: "Médico",
	ENTITY_NAME_PLURAL: "Médicos",
	SECTION: "medicos-section",
	TABLE: "medicos-table",
};
const SELECTORS = {
...
function renderDoctorsPage() {
	...
	let pageHtml = `
        ...
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center pt-3 pb-2 mb-3 border-bottom">
            <h1 class="h2 text-body mb-3 mb-md-0">Gestión de ${config.ENTITY_NAME_PLURAL}</h1>
            <div class="btn-toolbar">
                <button type="button" class="btn btn-primary" id="${SELECTORS.ADD_DOCTOR_BUTTON}">
                    <i class="fa fa-plus-circle me-1"></i>
                    Agregar ${config.ENTITY_NAME}
                </button>
            </div>
        </div>
        ...
        <div class="card">
            <div class="card-header">
                Listado de ${config.ENTITY_NAME_PLURAL}
            </div>
            <div class="card-body">
	...
	pageHtml = tpl.createSectionWrapper(config.SECTION, pageHtml);
...
function loadDoctors() {
	const tableContainer = document.getElementById(
		SELECTORS.DOCTORS_TABLE_CONTAINER,
	);
	...
	try {
		...
		if (doctors.length === 0) {
			tableContainer.innerHTML =
				`<p class="text-muted">No hay ${config.ENTITY_NAME_PLURAL} registrados.</p>`;
			return;
		}

		const headers = ["#", "Foto", "Nombre", "Especialidad", "Acciones"];
		const rowsHtml = doctors
			.map((doctor) => {
				const doctorForTable = {
					...doctor,
					fullName: fullName(doctor),
					specialtyName:
						state.specialties.find((s) => s.id === doctor.especialidadId)
							?.nombre || "N/A",
				};
		...
		tableContainer.innerHTML =
			`<p class="text-danger">Error al cargar los ${config.ENTITY_NAME_PLURAL}.</p>`;
	} finally {
		removeSpinner();
	}
}
...
async function handleSaveDoctor({ form, doctor = {}, modal }) {
	...
	formData.matriculaProfesional = Number(formData.matriculaProfesional);
...
function openDoctorModal(doctor = {}) {
	...
	const title = isEditing ? "Editar Médico" : "Agregar Médico";
  	const doctorData = isEditing
			? storageService.doctors.getById(doctor.id)
			: {};
		const form = tpl.createDoctorForm(
			doctorData,
			storageService.specialties.getAll(),
	 ...
	 createReusableModal({
		...
		footerButtons: [
			...
			{ text: "Guardar", className: "btn btn-primary", onClick: (_, modal) => {
...
function handleShowDoctor(doctorId) {
	...
	try {
		...
		console.error(error);
		modalBody.innerHTML = `<div class="alert alert-danger">${MESSAGES.ENTITY_LOAD_ERROR("información del profesional")}</div>`;
	}
}
...
function attachListeners() {
	document
		.getElementById(SELECTORS.ADD_DOCTOR_BUTTON)
