...
async function loadUsers(page = 1) {
	const tableContainer = document.getElementById(
		SELECTORS.USERS_TABLE_CONTAINER,
	);
	...
	let rowsHtml = Array.from({ length: PAGE_LIMIT })
		.fill(null)
	...
	try {
		...
		const { users, total } = await api.fetchUsers({ limit: PAGE_LIMIT, skip });
