...
const createDefaultInput = (props, value) => {
  ...
  input.addEventListener('input', () => {
    ...
    if (props.required && !value) {
      validationMessage = props.validationMessage || 'El campo es obligatorio.';
    } else if (props.pattern && !new RegExp(props.pattern).test(value)) {
      validationMessage =
        props.patternValidationMessage ||
        `Por favor, ingrese un formato válido.`;
    } else if (props.type === 'number' && Number.isNaN(value)) {
      validationMessage = props.numberValidationMessage || 'Por favor ingrese un numero valido';
    }
    ...
    input.classList[validationMessage ? 'add' : 'remove']('is-invalid');
  });
